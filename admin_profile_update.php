...
if(isset($_POST['update'])){
   ...
   $update_profile = $conn->prepare("UPDATE `studente` SET name = ?, email = ? WHERE id = ?");
   ...
   if(!empty($image)){
      ...
      if($image_size > 2000000){
         ...
         $update_image = $conn->prepare("UPDATE `studente` SET image = ? WHERE id = ?");
   ...
   if(!empty($previous_pass) || !empty($new_pass) || !empty($confirm_pass)){
      if($previous_pass != $old_pass){
         ...
         $update_password = $conn->prepare("UPDATE `studenti` SET password = ? WHERE id = ?");
...
<section class="update-profile-container">
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <div class="flex-btn">
         ...
         <a href="admin_dashboard.php" class="option-btn">go back</a>
      </div>
   </form>
